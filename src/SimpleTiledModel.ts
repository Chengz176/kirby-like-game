...
export const SimpleTiledModel = () => {
    ...
    const getAdjacentEdgeCount = (direction: number, currIndex: number, tile: number) => {
        let adjacentEdgeCount = 0;
        switch(direction) {
            case 0: 
                adjacentEdgeCount = edgesNCounts[currIndex][tileset[tile].edges[2]]; break;
            case 1: 
                adjacentEdgeCount = edgesECounts[currIndex][tileset[tile].edges[3]]; break;
            case 2: 
                adjacentEdgeCount = edgesSCounts[currIndex][tileset[tile].edges[0]]; break;
            case 3: 
                adjacentEdgeCount = edgesWCounts[currIndex][tileset[tile].edges[1]]; break;
            default: break;
        }
    ...
    const removeTile = (index: number, tileIndex: number, endTileIndex: number) => {
    ...
    const computeEntropy = (index: number) => {
        ...
        return Math.log(proportionSums[index]) - 
    ...
    const addPossibleFixedTiles = () => {
        ...
        for (let i = 0; i < indices.length; i++) {
            swap(indices,
                i,
                i + Math.floor(Math.random() * (indices.length - i))
            );
    ...
    const selectTile = () => {
        ...
        if (index !== undefined) {
            const comp = Math.random() * proportionSums[index];
            
            let temp = 0;
            for (let i = 0; i <= getEndOf(history.possibleTilesChanged[index]); i++) {
        ...
        while (queue.length > 0) {
            ...
            if (currIndex !== undefined) {
                ...
                for (let direction = 0; direction < 4; direction++) {
                    ...
                    if (adjacentIndex > -1) {
                        let hasImpact = false;
                        let end = getEndOf(history.possibleTilesChanged[adjacentIndex]);
                        for (let tileIndex = getEndOf(history.possibleTilesChanged[adjacentIndex]);
                             tileIndex > -1;
                             tileIndex--) {
                            const tile = possibleTiles[adjacentIndex][tileIndex];
                            if (getAdjacentEdgeCount(direction, currIndex, tile) === 0) {
                                removeTile(adjacentIndex, tileIndex, end);
                                end--;

                                if (edgesNCounts[adjacentIndex][tileset[tile].edges[0]] == 0 ||
                                    edgesECounts[adjacentIndex][tileset[tile].edges[1]] == 0 ||
                                    edgesSCounts[adjacentIndex][tileset[tile].edges[2]] == 0 ||
                                    edgesWCounts[adjacentIndex][tileset[tile].edges[3]] == 0) {
                                    hasImpact = true;
                                }
                            }
                        }
                        ...
                        if (end < getEndOf(history.possibleTilesChanged[adjacentIndex])) {
                            history.possibleTilesChanged[adjacentIndex].push(end);
                            getEndOf(history.indicesChanged).push(adjacentIndex);
                        }


                        if (getEndOf(history.possibleTilesChanged[adjacentIndex]) < 0) {
                            backTrack();
                            return false;
                        }
                    }
    ...
    return {
        ...
        setProportion: hasInitializedWrapper((newProportion: {[frame: number]: number}) => {
            ...
            for (const frame of Object.keys(newProportion)) {
                const frameNum = Number(frame);
                proportion[frameNum].weight = scaler(newProportion[frameNum]) / proportion[frameNum].count;
            }
        ...
        fixTiles: hasInitializedWrapper((fixedTiles: {index: number, tile: number}[]) => {
        ...
        generateMap: hasInitializedWrapper(() => {
            while (fixedTilesCount < size) {
                ...
                while (index_tile.index > -1 && !fixTile(index_tile.index, index_tile.tile)) {
